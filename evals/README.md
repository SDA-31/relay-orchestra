...
`transcripts.json` specifies ordered one-shot and multi-turn event traces. Runners should preserve each delivery batch, distinguish result notification from automatic wake, verify user-event priority, and assert scope, state transitions, continuity, and blocking-wait consent after every step. One-shot traces must finish `OFF` in their only turn even when blocked; bare explicit invocations require a later direct close answer.
